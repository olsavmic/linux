...
static const struct file_operations kbd_fops = { .owner = THIS_MODULE,
						 .open = kbd_open,
						 .release = kbd_release,
						 .read = kbd_read,
						 .write = kbd_write };
...
static int kbd_init(void)
{
	...
	err = request_irq(I8042_KBD_IRQ, kbd_interrupt_handle, IRQF_SHARED,
			  MODULE_NAME, &devs[0]);

	if (err != 0) {
		pr_err("Registering interrupt handler failed: %d\n", err);
